...
The first step is the get the data loaded and explore it:
...
In the documentation the units of the `len` variable are not provided. Based on
its values it seems likely that the measurements are in millimeters. The
...
## Part 3: Use hypothesis tests to compare tooth growth by `supp` and `dose`
